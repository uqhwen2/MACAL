...
# for seed in range(num_trial):
    ...
    for query_step in range(al_step):
        ...
        # Sample idx by the probability, not a strict sample by the criterion value. # this one used by the authors
        #scores = np.exp(scores)
        #p = scores / scores.sum()
        #idx = np.random.choice(range(len(p)), replace=False, p=p, size=num_of_samples)
