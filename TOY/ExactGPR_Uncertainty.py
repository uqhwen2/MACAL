...
def uncertainty_sampling(combine_x, T, pool, num_of_samples):
    model_1.eval()
    model_0.eval()
    
    pred_1, pred_0 = model_1(combine_x[pool]), model_0(combine_x[pool])
    pred_1_variance, pred_0_variance = pred_1.variance.sqrt(), pred_0.variance.sqrt()
    
    uncertainty_outputs = torch.concat([pred_1_variance.unsqueeze(-1), pred_0_variance.unsqueeze(-1)], dim=1)
    #print(uncertainty_outputs, T.unsqueeze(-1))
    ...
    threshold_top_k = np.partition(draw_dist.flatten(), -top_k)[-top_k]  # Calculate the threshold for the top 5 values instead of top 5%
    ...
    acquired_idx = pool[acquired_idx]
    ...
    return acquired_idx[:num_elements_to_select]
...
for query_step in range(25):
    ...
    acquired_index = uncertainty_sampling(combine_x=combine_x_train,
                                          T=T_train[idx_remaining],
                                          pool=idx_remaining, 
...
file_path = 'text_results/trueuncertainty/pehe_trueuncertainty_{}.csv'.format(args.seed)
