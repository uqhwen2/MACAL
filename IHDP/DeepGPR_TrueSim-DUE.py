...
def train_test_splitting(combine_x, combined_y, tau, T, test_size, seed, device):
    ...
    combined_y_train = torch.from_numpy(ihdp_train.y)  # No y normalization
    combined_y_valid = torch.from_numpy(ihdp_valid.y)

    tau_test = torch.from_numpy(ihdp_test.y)
...
#            print(max_values_remaining_1.shape)
