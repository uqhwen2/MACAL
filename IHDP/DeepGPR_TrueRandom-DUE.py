...
def training_nn(x_1, y_1, x_0, y_0, training_iter, combine_x_valid, combined_y_valid, T_valid):
    ...
    for i in range(training_iter):
        ...
        if last_value > valid_loss:
            ...
            }, 'model_selection/random_nnmodel_1.pth')

            torch.save({
                'model_state_dict': model_0.state_dict(),
            }, 'model_selection/random_nnmodel_0.pth')
    ...
    if True:
        ...
        checkpoint_1 = torch.load('model_selection/random_nnmodel_1.pth')
        model_1.load_state_dict(checkpoint_1['model_state_dict'])

        checkpoint_0 = torch.load('model_selection/random_nnmodel_0.pth')
...
#        list_1, list_0 = trt_ctr(T_train[combine_train_idx])
#        print("Number of data used for training in treated and control:", len(list_1), len(list_0))

#        list_1, list_0 = trt_ctr(T_train)
#        print("Number of data used for training in treated and control:", len(list_1), len(list_0))

         # Sort the array in ascending order
#        combine_train_idx = np.sort(combine_train_idx)
...
#        print('PRINTOUT:', mu_0.mean(0).shape, mu_1.mean(0).shape, tau_test.shape)
        ...
        if query_step!=0:
            if len(idx_sub_training_1)+len(idx_sub_training_0)==combine_x_train.shape[0]:
                ...
                print('Acquiring the treated and control:', len(treated_idx), len(control_idx))
